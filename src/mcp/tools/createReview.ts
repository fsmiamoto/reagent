...
export async function createReview(
  ...
  const {
    openBrowser: requestedOpenBrowser,
    _host: _ignoredHost,
    ...requestPayload
  } = input;
  ...
  try {
    ...
    const result = await apiFacade.post<CreateReviewResult>(
      "/reviews",
      requestPayload,
    );

    console.error(
      `[Reagent] Review session created: ${result.sessionId} (${result.filesCount} file(s))`,
    );
    ...
    console.error(
      `[Reagent] Browser opening disabled. Access review at: ${result.reviewUrl}`,
    );
